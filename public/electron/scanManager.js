...
const getResultsFolderPath = (scanId) => {
  const entry = scanHistory[scanId]
  if (!entry) return null
  // Folders registered via registerExistingReportFolder store the full
  // absolute path directly (they can live anywhere on disk, not just under
  // the configured export directory), while scans run through this app
  // store just the results folder name relative to exportDir.
  if (path.isAbsolute(entry)) return entry
  const exportDir = readUserDataFromFile().exportDir
  return path.join(exportDir, entry)
}

// Lets a user point LLM chat at a previously-generated LLM analysis report
// folder without re-running a scan. Only folders that actually contain the
// LLM analysis JSON artifacts (scanData.json.gz.b64, written when a scan
// runs with -g yes) are accepted — this is the same file llmAnalysis.js's
// loadArtifacts() requires to build a chat session.
const registerExistingReportFolder = (folderPath) => {
  try {
    if (!folderPath || !fs.existsSync(folderPath) || !fs.statSync(folderPath).isDirectory()) {
      return { success: false, error: 'Selected folder does not exist.' }
    }
    const marker = path.join(folderPath, 'scanData.json.gz.b64')
    if (!fs.existsSync(marker)) {
      return {
        success: false,
        error:
          'This folder does not contain LLM chat data. Choose a report folder generated by a previous "LLM chat (alpha)" scan.',
      }
    }
    const scanId = randomUUID()
    scanHistory[scanId] = folderPath
    return { success: true, scanId }
  } catch (error) {
    return { success: false, error: error?.message || 'Unable to use the selected folder.' }
  }
}
...
const init = (scanEvent) => {
  ...
  ipcMain.handle('registerExistingReportFolder', async (_event, folderPath) => {
    return registerExistingReportFolder(folderPath)
  })
