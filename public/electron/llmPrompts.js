...
**Framework/language docs rule:** Call \`search_language_and_frameworks\` before you (a) cite a specific React / Vue / Angular API surface (hooks, directives, decorators, lifecycle names, template syntax), (b) quote a JavaScript built-in signature or TS type/utility, (c) recommend a framework-specific accessibility utility (\`useId\`, \`v-bind\`, \`@angular/cdk/a11y\`, ARIA typings in TS), (d) answer a "how do I do X in \`<framework>\`?" question, or (e) draft example code that applies a CSS threshold / HTML attribute / ARIA property inside a known framework (React \`style\`/\`className\` prop, Vue \`:style\`/\`v-bind:class\`, Angular \`[style.min-width.px]\`/\`[class]\`) — the WCAG techniques show generic HTML/CSS, not framework-specific syntax. Pass an optional \`family\` filter (\`react\` | \`vue\` | \`angular\` | \`javascript\` | \`typescript\`) when the stack is unambiguous. Do NOT invent API names or syntax from memory — framework APIs churn fast; grounding in the returned snippet is cheaper than a wrong recommendation. When the user's question is purely conceptual ("what is WCAG 2.5.8", "why does target size matter") and no framework is named, \`search_wcag\` alone is enough.
...
const TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    description:
      'Hybrid (BM25 + vector) search over a local corpus of framework/language reference documentation — React (react.dev learn + reference), Vue (vuejs.org guide + API), Angular (angular.dev guide + reference), MDN JavaScript, and TypeScript (handbook + declaration files + reference). Call this when you need to ground the *implementation* side of a fix in an authoritative source: correct hook / directive / decorator / template-syntax / built-in signature / TS type — AND the framework-specific syntax for applying an HTML attribute, CSS threshold, or ARIA property (e.g. React `style={{ minWidth: 24 }}` vs Vue `:style` vs Angular `[style.min-width.px]`). Complements `search_wcag`, which grounds the *requirement* (SC thresholds, exemptions, DSS controls, technique ids) but shows only generic HTML/CSS — not framework idioms. **Querying tips:** both keyword-shaped and natural-language queries work. Pass an API name verbatim when you know it ("useId", "v-model", "signal", "Array.prototype.map", "keyof") — the tokenizer preserves dotted identifiers so "React.useState" and "Array.prototype.map" both work. When you don\'t know the exact API, phrase the question naturally ("how do I focus the first invalid field in an Angular form", "when should I use a mixin in TypeScript", "how do I set min-width in a React inline style") and the vector layer will match paraphrased concepts. If the stack is known, pass the optional `family` filter to keep the result set focused.',
    input_schema: {
      type: 'object',
      required: ['query'],
      properties: {
